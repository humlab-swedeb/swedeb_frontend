    ...
    <template v-else-if="speechesStore.speechesData.length > 0">
        <div>
            ...
            <q-table ref="SpeechTable" bordered flat :rows="rows" :columns="columns" row-key="id"
                ...
                :loading="speechesStore.isLoading || speechesStore.isPageLoading" class="bg-grey-2"
                @request="onRequest">
                <template v-slot:header="props">
                    <q-tr :props="props">
                        <q-th v-for="col in props.cols" :key="col.name" :props="props">
                            {{ col.label }}
                            <q-icon v-if="col.label === 'Anförande'" name="info_outline" color="accent"
                ...
                <template v-slot:body="props">
                    <q-tr :props="props" @click="expandRow(props)" class="cursor-pointer">
                        <q-td v-for="col in props.cols" :key="col.name" :props="props" class="bg-white"
                            ...
                            <q-item-label v-else-if="col.value === 'Okänd' || col.value === 'Okänt'"
